...
# (autojunk). 여기서는 실제 공문 형태로 재현해 보려 했으나 결과가 달라지는
# 경우를 찾지 못했다 — 흔한 줄이나 글자가 그만큼 많지 않기 때문이다.
# 그래도 끄고 쓴다. 맞는 설정이고 비용이 없다. 화면 비교(픽셀) 쪽은 사정이
# 달라서, 거기서는 빈칸이 열의 대부분이라 켜 두면 닮음이 0.997 → 0.455 로
# 무너지는 것을 실측했다.
